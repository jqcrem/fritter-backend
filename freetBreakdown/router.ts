...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    for (const content of contents){
      let freet = await FreetCollection.addOne(userId, content);
      freets.push(freet._id);
    }
    const freetBreakdown = await FreetBreakdownCollection.addOne(userId, freets);

    res.status(201).json({
      message: 'Your freetBreakdown was created successfully.',
      freet: util.constructFreetBreakdownResponse(freetBreakdown)
    });
  }
);
